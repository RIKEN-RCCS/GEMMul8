 ...
 * ----------
 * Hook targets:
 *   - cublas{S,D,C,Z}syrkx
 *   - cublas{S,D,C,Z}syrkx_64
 *
 * Notes:
...
static inline bool is_valid_syrkx_op(const cublasOperation_t trans) {
    return trans == CUBLAS_OP_N || trans == CUBLAS_OP_T;
}

static inline int64_t syrkx_lda_min(const cublasOperation_t trans, const int64_t n, const int64_t k) {
    return std::max<int64_t>(1, (trans == CUBLAS_OP_N) ? n : k);
}

static inline int64_t syrkx_ldc_min(const int64_t n) {
    return std::max<int64_t>(1, n);
}

static inline cublasStatus_t validate_syrkx_params(
    ...
    if (!is_valid_uplo(uplo) || !is_valid_syrkx_op(trans)) return CUBLAS_STATUS_INVALID_VALUE;
    if (!alpha || !beta || !A || !B || !C) return CUBLAS_STATUS_INVALID_VALUE;
    if (n < 0 || k < 0) return CUBLAS_STATUS_INVALID_VALUE;
    if (lda < syrkx_lda_min(trans, n, k)) return CUBLAS_STATUS_INVALID_VALUE;
    if (ldb < syrkx_lda_min(trans, n, k)) return CUBLAS_STATUS_INVALID_VALUE;
    if (ldc < syrkx_ldc_min(n)) return CUBLAS_STATUS_INVALID_VALUE;
    return CUBLAS_STATUS_SUCCESS;
}

template <typename T>
static inline gemmul8::hook::ScaledOperandKey make_syrkx_operand_key(
    const void *ptr,
    int64_t n,
    int64_t k,
    int64_t ld,
...
template <typename T> struct SyrkxTraits;

#define DEFINE_SYRKX_TRAITS(TYPE, GETENV_FN, SYM, SYM64)                                                \
    template <> struct SyrkxTraits<TYPE> {                                                              \
...
DEFINE_SYRKX_TRAITS(float, get_env_s, cublasSsyrkx, cublasSsyrkx_64)
DEFINE_SYRKX_TRAITS(double, get_env_d, cublasDsyrkx, cublasDsyrkx_64)
DEFINE_SYRKX_TRAITS(cuFloatComplex, get_env_c, cublasCsyrkx, cublasCsyrkx_64)
DEFINE_SYRKX_TRAITS(cuDoubleComplex, get_env_z, cublasZsyrkx, cublasZsyrkx_64)

#undef DEFINE_SYRKX_TRAITS

template <typename T>
static inline typename SyrkxTraits<T>::Fn real_syrkx() {
    using Fn     = typename SyrkxTraits<T>::Fn;
    static Fn fn = gemmul8::hook::load_real<Fn>(SyrkxTraits<T>::sym);
    return fn;
}

template <typename T>
static inline typename SyrkxTraits<T>::Fn64 real_syrkx_64() {
    using Fn     = typename SyrkxTraits<T>::Fn64;
    static Fn fn = gemmul8::hook::load_real<Fn>(SyrkxTraits<T>::sym_64);
    return fn;
}

template <typename T>
static inline cublasStatus_t call_gemmul8_syrkx(
    gemmul8::Backend backend,
    ...
    if (backend == gemmul8::Backend::INT8) {
        ...
        (void)gemmul8::syrkx<T, gemmul8::Backend::INT8>(
            handle,
            uplo, trans,
    ...
    (void)gemmul8::syrkxLt<T, gemmul8::Backend::FP8>(
        lt,
        uplo, trans,
        static_cast<size_t>(n), static_cast<size_t>(k),
        alpha,
...
template <typename T>
static inline size_t call_gemmul8_syrkx_workSize(
    gemmul8::Backend backend,
    int64_t n, int64_t k,
    int num_moduli,
    bool enable_skip_A,
    ...
    constexpr bool COMPLEX       = SyrkxTraits<T>::isComplex;
    constexpr gemmul8::Func FUNC = gemmul8::Func::syrkx;
    if (backend == gemmul8::Backend::INT8) {
...
template <typename T>
static inline cublasStatus_t run_gemmul8_syrkx_emulation(const SyrkxArgs<T> &a, const HookSyrkxEnv &env) {
    ...
    size_t wsizeA      = 0;
    size_t wsizeB      = 0;
    const size_t wsize = call_gemmul8_syrkx_workSize<T>(
        env.backend,
        a.n, a.k,
        env.num_moduli,
    ...
    const auto keyA = make_syrkx_operand_key<T>(
        a.A, a.n, a.k, a.lda, a.trans,
        env.num_moduli, env.fastmode, env.backend);

    const auto keyB = make_syrkx_operand_key<T>(
...
static inline cublasStatus_t syrkx_common_impl(
    cublasHandle_t handle,
    ...
    constexpr gemmul8::hook::HookOp OP = gemmul8::hook::HookOp::SYRKX;

    HookSyrkxEnv env{};
    SyrkxTraits<T>::get_env(OP, env.num_moduli, env.fastmode, env.enable_skipA, env.enable_skipB);
    ...
    const cublasStatus_t st_param = validate_syrkx_params(
    ...
    return run_gemmul8_syrkx_emulation<T>(args, env);
}

template <typename T>
static inline cublasStatus_t syrkx_impl(
    cublasHandle_t handle,
    cublasFillMode_t uplo,
    cublasOperation_t trans,
    int n, int k,
...
template <typename T>
static inline cublasStatus_t syrkx_64_impl(
    cublasHandle_t handle,
    cublasFillMode_t uplo,
    cublasOperation_t trans,
    int64_t n, int64_t k,
...
#define DEFINE_SYRKX_HOOK(NAME, TYPE)                                                            \
    extern "C" cublasStatus_t NAME(                                                              \
        ...
        if (gemmul8::hook::inside_hook()) {                                                      \
            auto fn = real_syrkx<TYPE>();                                                        \
            if (!fn) return CUBLAS_STATUS_NOT_INITIALIZED;                                       \
            return fn(handle, uplo, trans, n, k, alpha, A, lda, B, ldb, beta, C, ldc);           \
        }                                                                                        \
        gemmul8::hook::HookGuard guard;                                                          \
        return syrkx_impl<TYPE>(handle, uplo, trans, n, k, alpha, A, lda, B, ldb, beta, C, ldc); \
    }

#define DEFINE_SYRKX_64_HOOK(NAME, TYPE)                                                            \
    extern "C" cublasStatus_t NAME(                                                                 \
        cublasHandle_t handle,                                                                      \
        cublasFillMode_t uplo,                                                                      \
        cublasOperation_t trans,                                                                    \
        ...
        TYPE *C, int64_t ldc) {                                                                     \
        if (gemmul8::hook::inside_hook()) {                                                         \
            auto fn = real_syrkx_64<TYPE>();                                                        \
            if (!fn) return CUBLAS_STATUS_NOT_INITIALIZED;                                          \
            return fn(handle, uplo, trans, n, k, alpha, A, lda, B, ldb, beta, C, ldc);              \
        }                                                                                           \
        gemmul8::hook::HookGuard guard;                                                             \
        return syrkx_64_impl<TYPE>(handle, uplo, trans, n, k, alpha, A, lda, B, ldb, beta, C, ldc); \
    }

DEFINE_SYRKX_HOOK(cublasSsyrkx, float)
DEFINE_SYRKX_HOOK(cublasDsyrkx, double)
DEFINE_SYRKX_HOOK(cublasCsyrkx, cuFloatComplex)
DEFINE_SYRKX_HOOK(cublasZsyrkx, cuDoubleComplex)

DEFINE_SYRKX_64_HOOK(cublasSsyrkx_64, float)
DEFINE_SYRKX_64_HOOK(cublasDsyrkx_64, double)
DEFINE_SYRKX_64_HOOK(cublasCsyrkx_64, cuFloatComplex)
DEFINE_SYRKX_64_HOOK(cublasZsyrkx_64, cuDoubleComplex)

#undef DEFINE_SYRKX_HOOK
#undef DEFINE_SYRKX_64_HOOK
