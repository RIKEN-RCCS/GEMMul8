 ...
 * Hook targets:
 *   - cublas{C,Z}her2k_v2
 *   - cublas{C,Z}her2k_v2_64
 *
 * Notes:
 *   - HER2K is a complex Hermitian routine; there are no S/D hooks.
...
template <typename T> struct Her2kTraits;
template <typename T> struct Her2kTraitsScalar;
template <> struct Her2kTraitsScalar<cuFloatComplex> {
    using type = float;
};
template <> struct Her2kTraitsScalar<cuDoubleComplex> {
    using type = double;
};
template <typename T> using Her2kTraitsScalarT = typename Her2kTraitsScalar<T>::type;
...
static inline bool is_valid_her2k_op(const cublasOperation_t trans) {
    return trans == CUBLAS_OP_N || trans == CUBLAS_OP_C;
}

static inline int64_t her2k_lda_min(const cublasOperation_t trans, const int64_t n, const int64_t k) {
    return std::max<int64_t>(1, (trans == CUBLAS_OP_N) ? n : k);
}

static inline int64_t her2k_ldc_min(const int64_t n) {
    return std::max<int64_t>(1, n);
}

static inline cublasStatus_t validate_her2k_params(
    ...
    if (!is_valid_uplo(uplo) || !is_valid_her2k_op(trans)) return CUBLAS_STATUS_INVALID_VALUE;
    if (!alpha || !beta || !A || !B || !C) return CUBLAS_STATUS_INVALID_VALUE;
    if (n < 0 || k < 0) return CUBLAS_STATUS_INVALID_VALUE;
    if (lda < her2k_lda_min(trans, n, k)) return CUBLAS_STATUS_INVALID_VALUE;
    if (ldb < her2k_lda_min(trans, n, k)) return CUBLAS_STATUS_INVALID_VALUE;
    if (ldc < her2k_ldc_min(n)) return CUBLAS_STATUS_INVALID_VALUE;
    return CUBLAS_STATUS_SUCCESS;
}

template <typename T>
static inline gemmul8::hook::ScaledOperandKey make_her2k_operand_key(
    const void *ptr,
    int64_t n,
    int64_t k,
    int64_t ld,
...
#define DEFINE_HER2K_TRAITS(TYPE, GETENV_FN, SYM, SYM64)                                                \
    template <> struct Her2kTraits<TYPE> {                                                              \
        ...
        using Scalar = Her2kTraitsScalarT<TYPE>;                                                        \
...
DEFINE_HER2K_TRAITS(cuFloatComplex, get_env_c, cublasCher2k_v2, cublasCher2k_v2_64)
DEFINE_HER2K_TRAITS(cuDoubleComplex, get_env_z, cublasZher2k_v2, cublasZher2k_v2_64)

#undef DEFINE_HER2K_TRAITS

template <typename T>
static inline typename Her2kTraits<T>::Fn real_her2k_v2() {
    using Fn     = typename Her2kTraits<T>::Fn;
    static Fn fn = gemmul8::hook::load_real<Fn>(Her2kTraits<T>::sym);
    return fn;
}

template <typename T>
static inline typename Her2kTraits<T>::Fn64 real_her2k_v2_64() {
    using Fn     = typename Her2kTraits<T>::Fn64;
    static Fn fn = gemmul8::hook::load_real<Fn>(Her2kTraits<T>::sym_64);
    return fn;
}

template <typename T>
static inline cublasStatus_t call_gemmul8_her2k(
    gemmul8::Backend backend,
    ...
    const Her2kTraitsScalarT<T> *beta,
    T *C, int64_t ldc,
    ...
    if (backend == gemmul8::Backend::INT8) {
        ...
        (void)gemmul8::her2k<T, gemmul8::Backend::INT8>(
            handle,
            uplo, trans,
    ...
    (void)gemmul8::her2kLt<T, gemmul8::Backend::FP8>(
        lt,
        uplo, trans,
        static_cast<size_t>(n), static_cast<size_t>(k),
        alpha,
...
template <typename T>
static inline size_t call_gemmul8_her2k_workSize(
    gemmul8::Backend backend,
    int64_t n, int64_t k,
    int num_moduli,
    bool enable_skip_A,
    ...
    constexpr gemmul8::Func FUNC = gemmul8::Func::her2k;
    if (backend == gemmul8::Backend::INT8) {
...
struct Her2kArgs {
    ...
    const T *B                        = nullptr;
    int64_t ldb                       = 0;
    const Her2kTraitsScalarT<T> *beta = nullptr;
    T *C                              = nullptr;
    int64_t ldc                       = 0;
};
...
template <typename T>
static inline cublasStatus_t run_gemmul8_her2k_emulation(const Her2kArgs<T> &a, const HookHer2kEnv &env) {
    ...
    size_t wsizeA      = 0;
    size_t wsizeB      = 0;
    const size_t wsize = call_gemmul8_her2k_workSize<T>(
        env.backend,
        a.n, a.k,
        env.num_moduli,
    ...
    const auto keyA = make_her2k_operand_key<T>(
        a.A, a.n, a.k, a.lda, a.trans,
        env.num_moduli, env.fastmode, env.backend);

    const auto keyB = make_her2k_operand_key<T>(
...
static inline cublasStatus_t her2k_common_impl(
    cublasHandle_t handle,
    ...
    constexpr gemmul8::hook::HookOp OP = gemmul8::hook::HookOp::HER2K;

    HookHer2kEnv env{};
    Her2kTraits<T>::get_env(OP, env.num_moduli, env.fastmode, env.enable_skipA, env.enable_skipB);
    ...
    const cublasStatus_t st_param = validate_her2k_params(
    ...
    return run_gemmul8_her2k_emulation<T>(args, env);
}

template <typename T>
static inline cublasStatus_t her2k_v2_impl(
    cublasHandle_t handle,
    cublasFillMode_t uplo,
    cublasOperation_t trans,
    int n, int k,
...
template <typename T>
static inline cublasStatus_t her2k_v2_64_impl(
    cublasHandle_t handle,
    cublasFillMode_t uplo,
    cublasOperation_t trans,
    int64_t n, int64_t k,
...
#define DEFINE_HER2K_V2_HOOK(NAME, TYPE)                                                            \
    extern "C" cublasStatus_t NAME(                                                                 \
        ...
        const Her2kTraitsScalarT<TYPE> *beta,                                                       \
        TYPE *C, int ldc) {                                                                         \
        if (gemmul8::hook::inside_hook()) {                                                         \
            auto fn = real_her2k_v2<TYPE>();                                                        \
            if (!fn) return CUBLAS_STATUS_NOT_INITIALIZED;                                          \
            return fn(handle, uplo, trans, n, k, alpha, A, lda, B, ldb, beta, C, ldc);              \
        }                                                                                           \
        gemmul8::hook::HookGuard guard;                                                             \
        return her2k_v2_impl<TYPE>(handle, uplo, trans, n, k, alpha, A, lda, B, ldb, beta, C, ldc); \
    }

#define DEFINE_HER2K_V2_64_HOOK(NAME, TYPE)                                                            \
    extern "C" cublasStatus_t NAME(                                                                    \
        cublasHandle_t handle,                                                                         \
        cublasFillMode_t uplo,                                                                         \
        cublasOperation_t trans,                                                                       \
        ...
        TYPE *C, int64_t ldc) {                                                                        \
        if (gemmul8::hook::inside_hook()) {                                                            \
            auto fn = real_her2k_v2_64<TYPE>();                                                        \
            if (!fn) return CUBLAS_STATUS_NOT_INITIALIZED;                                             \
            return fn(handle, uplo, trans, n, k, alpha, A, lda, B, ldb, beta, C, ldc);                 \
        }                                                                                              \
        gemmul8::hook::HookGuard guard;                                                                \
        return her2k_v2_64_impl<TYPE>(handle, uplo, trans, n, k, alpha, A, lda, B, ldb, beta, C, ldc); \
    }

DEFINE_HER2K_V2_HOOK(cublasCher2k_v2, cuFloatComplex)
DEFINE_HER2K_V2_HOOK(cublasZher2k_v2, cuDoubleComplex)

DEFINE_HER2K_V2_64_HOOK(cublasCher2k_v2_64, cuFloatComplex)
DEFINE_HER2K_V2_64_HOOK(cublasZher2k_v2_64, cuDoubleComplex)

#undef DEFINE_HER2K_V2_HOOK
#undef DEFINE_HER2K_V2_64_HOOK
