 ...
 * ---------
 * Hook targets:
 *   - cublas{S,D,C,Z}symm_v2
 *   - cublas{S,D,C,Z}symm_v2_64
 *
 * Notes:
 *   - cublas{S,D,C,Z}symm and cublas{S,D,C,Z}symm_64 are not defined here
...
static inline int64_t symm_order_A(const cublasSideMode_t side, const int64_t m, const int64_t n) {
    return (side == CUBLAS_SIDE_LEFT) ? m : n;
}

static inline int64_t symm_lda_min(const cublasSideMode_t side, const int64_t m, const int64_t n) {
    return std::max<int64_t>(1, symm_order_A(side, m, n));
}

static inline int64_t symm_ldb_min(const int64_t m) {
    return std::max<int64_t>(1, m);
}

static inline int64_t symm_ldc_min(const int64_t m) {
    return std::max<int64_t>(1, m);
}

static inline cublasStatus_t validate_symm_params(
    ...
    if (lda < symm_lda_min(side, m, n)) return CUBLAS_STATUS_INVALID_VALUE;
    if (ldb < symm_ldb_min(m)) return CUBLAS_STATUS_INVALID_VALUE;
    if (ldc < symm_ldc_min(m)) return CUBLAS_STATUS_INVALID_VALUE;
    return CUBLAS_STATUS_SUCCESS;
}

template <typename T>
static inline gemmul8::hook::ScaledOperandKey make_symm_A_key(
    const void *A,
    int64_t order,
    int64_t lda,
    ...
    return gemmul8::hook::ScaledOperandKey{
        ...
        .matrix_kind = gemmul8::hook::MatrixKind::Symmetric,
...
template <typename T>
static inline gemmul8::hook::ScaledOperandKey make_symm_B_key(
    const void *B,
    int64_t m,
    int64_t n,
    int64_t ldb,
...
template <typename T> struct SymmTraits;

template <> struct SymmTraits<float> {
    static constexpr bool isComplex = false;
    static inline void get_env(gemmul8::hook::HookOp op, int &nm, bool &fm, bool &enA, bool &enB) {
        gemmul8::hook::get_env_s(op, nm, fm, enA, enB);
    }
    using SymmV2Fn = cublasStatus_t (*)(
        cublasHandle_t, cublasSideMode_t, cublasFillMode_t,
        int, int,
        const float *, const float *, int,
        const float *, int,
        const float *, float *, int);
    using SymmV2Fn64 = cublasStatus_t (*)(
        cublasHandle_t, cublasSideMode_t, cublasFillMode_t,
        int64_t, int64_t,
        const float *, const float *, int64_t,
        const float *, int64_t,
        const float *, float *, int64_t);
    static constexpr const char *symm_v2_sym    = STR(cublasSsymm_v2);
    static constexpr const char *symm_v2_64_sym = STR(cublasSsymm_v2_64);
};
template <> struct SymmTraits<double> {
    static constexpr bool isComplex = false;
    static inline void get_env(gemmul8::hook::HookOp op, int &nm, bool &fm, bool &enA, bool &enB) {
        gemmul8::hook::get_env_d(op, nm, fm, enA, enB);
    }
    using SymmV2Fn = cublasStatus_t (*)(
        cublasHandle_t, cublasSideMode_t, cublasFillMode_t,
        int, int,
    ...
    static constexpr const char *symm_v2_sym    = STR(cublasDsymm_v2);
    static constexpr const char *symm_v2_64_sym = STR(cublasDsymm_v2_64);
};
template <> struct SymmTraits<cuFloatComplex> {
    ...
    static constexpr const char *symm_v2_sym    = STR(cublasCsymm_v2);
    static constexpr const char *symm_v2_64_sym = STR(cublasCsymm_v2_64);
};
template <> struct SymmTraits<cuDoubleComplex> {
    static constexpr bool isComplex = true;
    static inline void get_env(gemmul8::hook::HookOp op, int &nm, bool &fm, bool &enA, bool &enB) {
    ...
    static constexpr const char *symm_v2_sym    = STR(cublasZsymm_v2);
    static constexpr const char *symm_v2_64_sym = STR(cublasZsymm_v2_64);
};

template <typename T>
static inline typename SymmTraits<T>::SymmV2Fn real_symm_v2() {
    using Fn     = typename SymmTraits<T>::SymmV2Fn;
    static Fn fn = gemmul8::hook::load_real<Fn>(SymmTraits<T>::symm_v2_sym);
    return fn;
}

template <typename T>
static inline typename SymmTraits<T>::SymmV2Fn64 real_symm_v2_64() {
    using Fn     = typename SymmTraits<T>::SymmV2Fn64;
    static Fn fn = gemmul8::hook::load_real<Fn>(SymmTraits<T>::symm_v2_64_sym);
    return fn;
}

// ---- SYMM call wrapper (INT8 or FP8) ----
template <typename T>
static inline cublasStatus_t call_gemmul8_symm(
    gemmul8::Backend backend,
    ...
    if (backend == gemmul8::Backend::INT8) {
        ...
        (void)gemmul8::symm<T, gemmul8::Backend::INT8>(
            handle,
            side, uplo,
    ...
    (void)gemmul8::symmLt<T, gemmul8::Backend::FP8>(
        lt,
        side, uplo,
        static_cast<size_t>(m), static_cast<size_t>(n),
        alpha,
...
template <typename T>
static inline size_t call_gemmul8_symm_workSize(
    gemmul8::Backend backend,
    ...
    constexpr bool COMPLEX       = SymmTraits<T>::isComplex;
    constexpr gemmul8::Func FUNC = gemmul8::Func::symm;
...
template <typename T>
static inline cublasStatus_t run_gemmul8_symm_emulation(const SymmArgs<T> &a, const HookSymmEnv &env) {
    ...
    const int64_t orderA = symm_order_A(a.side, a.m, a.n);

    const auto keyA = make_symm_A_key<T>(
        a.A, orderA, a.lda, a.uplo,
        env.num_moduli, env.fastmode, env.backend);

    const auto keyB = make_symm_B_key<T>(
        a.B, a.m, a.n, a.ldb,
        env.num_moduli, env.fastmode, env.backend);
...
static inline cublasStatus_t symm_common_impl(
    cublasHandle_t handle,
    cublasSideMode_t side,
    cublasFillMode_t uplo,
    int64_t m, int64_t n,
    ...
    const gemmul8::hook::HookOp OP = gemmul8::hook::hook_op_from_symm_side(side);

    HookSymmEnv env{};
    SymmTraits<T>::get_env(OP, env.num_moduli, env.fastmode, env.enable_skipA, env.enable_skipB);
    ...
    const cublasStatus_t st_param = validate_symm_params(
    ...
    return run_gemmul8_symm_emulation<T>(args, env);
}

template <typename T>
static inline cublasStatus_t symm_v2_impl(
    cublasHandle_t handle,
    cublasSideMode_t side,
    cublasFillMode_t uplo,
    int m, int n,
...
template <typename T>
static inline cublasStatus_t symm_v2_64_impl(
    cublasHandle_t handle,
    cublasSideMode_t side,
    cublasFillMode_t uplo,
    int64_t m, int64_t n,
...
#define DEFINE_SYMM_V2_HOOK(NAME, TYPE)                                                           \
    extern "C" cublasStatus_t NAME(                                                               \
        ...
        if (gemmul8::hook::inside_hook()) {                                                       \
            auto fn = real_symm_v2<TYPE>();                                                       \
            if (!fn) return CUBLAS_STATUS_NOT_INITIALIZED;                                        \
            return fn(handle, side, uplo, m, n, alpha, A, lda, B, ldb, beta, C, ldc);             \
        }                                                                                         \
        gemmul8::hook::HookGuard guard;                                                           \
        return symm_v2_impl<TYPE>(handle, side, uplo, m, n, alpha, A, lda, B, ldb, beta, C, ldc); \
    }

#define DEFINE_SYMM_V2_64_HOOK(NAME, TYPE)                                                           \
    extern "C" cublasStatus_t NAME(                                                                  \
        cublasHandle_t handle,                                                                       \
        cublasSideMode_t side,                                                                       \
        cublasFillMode_t uplo,                                                                       \
        ...
        TYPE *C, int64_t ldc) {                                                                      \
        if (gemmul8::hook::inside_hook()) {                                                          \
            auto fn = real_symm_v2_64<TYPE>();                                                       \
            if (!fn) return CUBLAS_STATUS_NOT_INITIALIZED;                                           \
            return fn(handle, side, uplo, m, n, alpha, A, lda, B, ldb, beta, C, ldc);                \
        }                                                                                            \
        gemmul8::hook::HookGuard guard;                                                              \
        return symm_v2_64_impl<TYPE>(handle, side, uplo, m, n, alpha, A, lda, B, ldb, beta, C, ldc); \
    }

DEFINE_SYMM_V2_HOOK(cublasSsymm_v2, float)
DEFINE_SYMM_V2_HOOK(cublasDsymm_v2, double)
DEFINE_SYMM_V2_HOOK(cublasCsymm_v2, cuFloatComplex)
DEFINE_SYMM_V2_HOOK(cublasZsymm_v2, cuDoubleComplex)

DEFINE_SYMM_V2_64_HOOK(cublasSsymm_v2_64, float)
DEFINE_SYMM_V2_64_HOOK(cublasDsymm_v2_64, double)
DEFINE_SYMM_V2_64_HOOK(cublasCsymm_v2_64, cuFloatComplex)
DEFINE_SYMM_V2_64_HOOK(cublasZsymm_v2_64, cuDoubleComplex)

#undef DEFINE_SYMM_V2_HOOK
#undef DEFINE_SYMM_V2_64_HOOK
