 ...
 *
 * This interface follows the cublasXtrmm / hipblasXtrmm argument convention,
 ...
 *
 *   work:
 *     Workspace pointer.  If work == nullptr, the TRMM operation is not
 ...
 *
 *   If work != nullptr:
 *     Executes TRMM and returns elapsed times (in seconds) of the four internal phases:
 ...
 *
 *   If work == nullptr:
 *     Does not execute TRMM and returns required workspace sizes in bytes:
 ...
 * Lt variant:
 *
 *   trmmLt() uses a cublasLtHandle_t / hipblasLtHandle_t and appends a stream
 *   argument at the end.
 */
#pragma once
#include "types.hpp"
...
// Alternative template-argument order:
template <Backend BACKEND, typename TA, typename TB = TA, typename TC = TA>
inline std::vector<double> trmmLt(
    cublasLtHandle_t handle,
    cublasSideMode_t side, cublasFillMode_t uplo,
    cublasOperation_t trans, cublasDiagType_t diag,
    size_t m, size_t n,
    ...
    return trmmLt<TA, BACKEND, TB, TC>(
        handle, side, uplo, trans, diag, m, n,
        alpha, A, lda, B, ldb, C, ldc,
        num_moduli, fastmode,
        work, workA, workB,
