 ...
 *
 * This interface follows the cublasXgemm / hipblasXgemm argument convention,
 ...
 *
 *     gemmul8::gemm<TA, gemmul8::Backend::INT8, TB, TC>(...);
 *     gemmul8::gemm<gemmul8::Backend::INT8, TA, TB, TC>(...);
 *
 ...
 *
 *   work:
 *     Workspace pointer.  If work == nullptr, the GEMM operation is not
 ...
 *
 *   If work != nullptr:
 *     Executes GEMM and returns elapsed times (in seconds) of the four internal phases:
 ...
 *
 *   If work == nullptr:
 *     Does not execute GEMM and returns required workspace sizes in bytes:
 ...
 * Lt variant:
 *
 *   gemmLt() uses a cublasLtHandle_t / hipblasLtHandle_t and appends a stream
 *   argument at the end.
 */
...
// Alternative template-argument order:
template <Backend BACKEND, typename TA, typename TB = TA, typename TC = TA>
inline std::vector<double> gemmLt(
    cublasLtHandle_t handle,
    cublasOperation_t op_A, cublasOperation_t op_B,
    size_t m, size_t n, size_t k,
    const TC *alpha,
    ...
    return gemmLt<TA, BACKEND, TB, TC>(
        handle, op_A, op_B, m, n, k,
        alpha, A, lda, B, ldb, beta, C, ldc,
        num_moduli, fastmode,
        work, workA, workB,
