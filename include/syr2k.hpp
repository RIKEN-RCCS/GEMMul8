 ...
 * SYR2K
 * -----
 * Computes a symmetric rank-2k update:
 *
 ...
 *
 * This interface follows the cublasXsyr2k / hipblasXsyr2k argument convention,
 ...
 *
 *     gemmul8::syr2k<TA, gemmul8::Backend::INT8, TB, TC>(...);
 *     gemmul8::syr2k<gemmul8::Backend::INT8, TA, TB, TC>(...);
 *
 *   If TA, TB, and TC can be deduced from the function arguments, the
 *   alternative order can be shortened to:
 *
 *     gemmul8::syr2k<gemmul8::Backend::INT8>(...);
 *
 ...
 *
 *   work:
 *     Workspace pointer.  If work == nullptr, the SYR2K operation is not
 ...
 *
 *   If work != nullptr:
 *     Executes SYR2K and returns elapsed times (in seconds) of the four internal phases:
 ...
 *
 *   If work == nullptr:
 *     Does not execute SYR2K and returns required workspace sizes in bytes:
 ...
 * Lt variant:
 *
 *   syr2kLt() uses a cublasLtHandle_t / hipblasLtHandle_t and appends a stream
 *   argument at the end.
 */
#pragma once
#include "types.hpp"
...
inline std::vector<double> syr2k(
    cublasHandle_t handle,
    cublasFillMode_t uplo, cublasOperation_t trans,
    size_t n, size_t k,
    const TC *alpha,
...
// Alternative template-argument order:
template <Backend BACKEND, typename TA, typename TB = TA, typename TC = TA>
inline std::vector<double> syr2kLt(
    cublasLtHandle_t handle,
    cublasFillMode_t uplo, cublasOperation_t trans,
    size_t n, size_t k,
    const TC *alpha,
    ...
    return syr2kLt<TA, BACKEND, TB, TC>(
        handle, uplo, trans, n, k,
        alpha, A, lda, B, ldb, beta, C, ldc,
        num_moduli, fastmode,
        work, workA, workB,
