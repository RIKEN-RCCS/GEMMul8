 ...
 * TRTRMM
 * ------
 * Computes a triangular-by-triangular matrix product:
 *
 *   C := alpha * op(A) * op(B) + beta * C.
 *
 * A and B are triangular matrices.  The triangles specified for A and B are
 * referenced.  If the corresponding diag argument is CUBLAS_DIAG_UNIT / HIPBLAS_DIAG_UNIT,
 * the diagonal entries are treated as one.
 *
 * This interface follows the BLAS-like triangular matrix operation convention,
 ...
 *
 *     gemmul8::trtrmm<TA, gemmul8::Backend::INT8, TB, TC>(...);
 *     gemmul8::trtrmm<gemmul8::Backend::INT8, TA, TB, TC>(...);
 *
 *   If TA, TB, and TC can be deduced from the function arguments, the
 *   alternative order can be shortened to:
 *
 *     gemmul8::trtrmm<gemmul8::Backend::INT8>(...);
 *
 ...
 *
 *   work:
 *     Workspace pointer.  If work == nullptr, the TRTRMM operation is not
 ...
 *
 *   If work != nullptr:
 *     Executes TRTRMM and returns elapsed times (in seconds) of the four internal phases:
 ...
 *
 *   If work == nullptr:
 *     Does not execute TRTRMM and returns required workspace sizes in bytes:
 ...
 * Lt variant:
 *
 *   trtrmmLt() uses a cublasLtHandle_t / hipblasLtHandle_t and appends a stream
 *   argument at the end.
 */
#pragma once
#include "types.hpp"
...
std::vector<double> trtrmm(
    ...
    size_t n,
    const TC *alpha,
    const TA *const A, size_t lda, // n x n triangular
    const TB *const B, size_t ldb, // n x n triangular
    const TC *beta,
...
inline std::vector<double> trtrmm(
    cublasHandle_t handle,
    cublasFillMode_t uplo_A, cublasFillMode_t uplo_B,
    cublasOperation_t trans_A, cublasOperation_t trans_B,
    cublasDiagType_t diag_A, cublasDiagType_t diag_B,
    ...
    return trtrmm<TA, BACKEND, TB, TC>(
        handle, uplo_A, uplo_B, trans_A, trans_B, diag_A, diag_B, n,
...
// Alternative template-argument order:
template <Backend BACKEND, typename TA, typename TB = TA, typename TC = TA>
inline std::vector<double> trtrmmLt(
    cublasLtHandle_t handle,
    cublasFillMode_t uplo_A, cublasFillMode_t uplo_B,
    cublasOperation_t trans_A, cublasOperation_t trans_B,
    cublasDiagType_t diag_A, cublasDiagType_t diag_B,
    ...
    return trtrmmLt<TA, BACKEND, TB, TC>(
        handle, uplo_A, uplo_B, trans_A, trans_B, diag_A, diag_B, n,
        alpha, A, lda, B, ldb, beta, C, ldc,
        num_moduli, fastmode,
        work, workA, workB,
...
std::vector<double> trtrmm(
    ...
    hipblasFillMode_t uplo_A, hipblasFillMode_t uplo_B,
    hipblasOperation_t trans_A, hipblasOperation_t trans_B,
    hipblasDiagType_t diag_A, hipblasDiagType_t diag_B,
    size_t n,
    const TC *alpha,
    const TA *const A, size_t lda, // n x n triangular
    const TB *const B, size_t ldb, // n x n triangular
