 ...
 * SYRKX
 * -----
 * Computes a symmetric rank-k update with two input matrices:
 *
 ...
 *
 * This interface follows the cublasXsyrkx / hipblasXsyrkx argument convention,
 ...
 *
 *     gemmul8::syrkx<TA, gemmul8::Backend::INT8, TB, TC>(...);
 *     gemmul8::syrkx<gemmul8::Backend::INT8, TA, TB, TC>(...);
 *
 *   If TA, TB, and TC can be deduced from the function arguments, the
 *   alternative order can be shortened to:
 *
 *     gemmul8::syrkx<gemmul8::Backend::INT8>(...);
 *
 ...
 *
 *   work:
 *     Workspace pointer.  If work == nullptr, the SYRKX operation is not
 ...
 *
 *   If work != nullptr:
 *     Executes SYRKX and returns elapsed times (in seconds) of the four internal phases:
 ...
 *
 *   If work == nullptr:
 *     Does not execute SYRKX and returns required workspace sizes in bytes:
 ...
 * Lt variant:
 *
 *   syrkxLt() uses a cublasLtHandle_t / hipblasLtHandle_t and appends a stream
 *   argument at the end.
 */
#pragma once
#include "types.hpp"
...
inline std::vector<double> syrkx(
    cublasHandle_t handle,
    cublasFillMode_t uplo, cublasOperation_t trans,
    size_t n, size_t k,
    const TC *alpha,
...
// Alternative template-argument order:
template <Backend BACKEND, typename TA, typename TB = TA, typename TC = TA>
inline std::vector<double> syrkxLt(
    cublasLtHandle_t handle,
    cublasFillMode_t uplo, cublasOperation_t trans,
    size_t n, size_t k,
    const TC *alpha,
    ...
    return syrkxLt<TA, BACKEND, TB, TC>(
        handle, uplo, trans, n, k,
        alpha, A, lda, B, ldb, beta, C, ldc,
        num_moduli, fastmode,
        work, workA, workB,
