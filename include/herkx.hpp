 ...
 * HERKX
 * -----
 * Computes a Hermitian rank-k update with two input matrices:
 *
 ...
 *
 * This interface follows the cublasXherkx / hipblasXherkx argument convention,
 ...
 *
 *     gemmul8::herkx<TA, gemmul8::Backend::INT8, TB, TC>(...);
 *     gemmul8::herkx<gemmul8::Backend::INT8, TA, TB, TC>(...);
 *
 *   If TA, TB, and TC can be deduced from the function arguments, the
 *   alternative order can be shortened to:
 *
 *     gemmul8::herkx<gemmul8::Backend::INT8>(...);
 *
 ...
 *
 *   work:
 *     Workspace pointer.  If work == nullptr, the HERKX operation is not
 ...
 *
 *   If work != nullptr:
 *     Executes HERKX and returns elapsed times (in seconds) of the four internal phases:
 ...
 *
 *   If work == nullptr:
 *     Does not execute HERKX and returns required workspace sizes in bytes:
 ...
 * Lt variant:
 *
 *   herkxLt() uses a cublasLtHandle_t / hipblasLtHandle_t and appends a stream
 *   argument at the end.
 */
#pragma once
#include "types.hpp"
...
inline std::vector<double> herkx(
    cublasHandle_t handle,
    cublasFillMode_t uplo, cublasOperation_t trans,
    size_t n, size_t k,
    const TC *alpha,
...
// Alternative template-argument order:
template <Backend BACKEND, typename TA, typename TB = TA, typename TC = TA>
inline std::vector<double> herkxLt(
    cublasLtHandle_t handle,
    cublasFillMode_t uplo, cublasOperation_t trans,
    size_t n, size_t k,
    const TC *alpha,
    ...
    return herkxLt<TA, BACKEND, TB, TC>(
        handle, uplo, trans, n, k,
        alpha, A, lda, B, ldb, beta, C, ldc,
        num_moduli, fastmode,
        work, workA, workB,
