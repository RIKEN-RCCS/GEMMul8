...
std::vector<double> run(
    ...
    if (!memory_saving || full_worksize <= limit) {
        return core::oz2_core<Func::syrkx, TA, TB, TC,
    ...
    if (k > block.yB) {
        ...
        if (!one) {
            assert(false && "Failed to create beta=1 scalar for blocked SYRKX.");
            return std::vector<double>(4, 0.0);
        }
    }
    ...
    core::blocking::for_each_rankk_tile(
        ...
        [&](size_t i, size_t ni, size_t p, size_t kp, bool first) {
            ...
            const auto t = core::oz2_core<Func::syrkx, TA, TB, TC,
                                          BACKEND, NUM_MODULI, TC, TC,
                                          FULL, FULL, UF, UF, DN, DN, UPLO_C>(
                handle, trans, op_B, ni, ni, kp,
                alpha, Ai, lda, Bi, ldb, first ? beta : one, Cii, ldc,
