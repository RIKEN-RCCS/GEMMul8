...
#include "time_syr2k.hpp"

namespace bench::time::syr2k {

inline constexpr gemmul8::Func func = gemmul8::Func::syr2k;
...
void check_time(
    ...
    if (trans == CUBLAS_OP_C) {
        assert(false && "SYR2K with trans = C is not supported.");
        return;
    }
    ...
    for (auto &n : N_list) {
        ...
        for (auto &k : K_list) {
            ...
            makemat::randmat<T>(rowsAB, colsAB, B, phi, seedB, stream);
            ...
            if (run_oz2_i8) {
                for (unsigned num_moduli = num_moduli_min; num_moduli <= num_moduli_max; ++num_moduli) {
                    ...
                    evaluate_time<isOzaki2>(
                        ...
                        comp_cost,
                        funcname,
                        [&]() { return gemmul8::syr2k<T, backend, T, T>(
            ...
            if (run_oz2_f8) {
                for (unsigned num_moduli = num_moduli_min; num_moduli <= num_moduli_max; ++num_moduli) {
                    ...
                    evaluate_time<isOzaki2>(
                        ...
                        comp_cost,
                        funcname,
                        [&]() { return gemmul8::syr2kLt<T, backend, T, T>(
            ...
            if (run_oz1_i8) {
                for (auto &num_slice : oz1_slice_list) {
                    ...
                    evaluate_time<isOzaki2>(
                        ...
                        comp_cost,
                        funcname,
                        [&]() { CHECK_CUBLAS(testTraits<T>::syr2k(
