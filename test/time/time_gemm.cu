...
#include "time_gemm.hpp"

namespace bench::time::gemm {

inline constexpr gemmul8::Func func = gemmul8::Func::gemm;
...
void check_time(
    ...
    PRINT(outFile,
          "phi,m,n,k,function,TFLOPS,total_time[sec],scaling,low_prec_gemm,mod_hi2mid,undo_scaling,");
    ...
    for (auto &m : N_list) {
        ...
        for (auto &k : K_list) {
            ...
            const size_t rowsA = (transa == CUBLAS_OP_N) ? m : k;
            const size_t colsA = (transa == CUBLAS_OP_N) ? k : m;
            const size_t rowsB = (transb == CUBLAS_OP_N) ? k : n;
            const size_t colsB = (transb == CUBLAS_OP_N) ? n : k;

            const size_t lda = rowsA;
            const size_t ldb = rowsB;
            const size_t ldc = m;

            const size_t size_A = rowsA * colsA * sizeof(T);
            const size_t size_B = rowsB * colsB * sizeof(T);
            ...
            makemat::randmat<T>(rowsB, colsB, B, phi, seedB, stream);
            ...
            if (run_oz2_i8) {
                for (unsigned num_moduli = num_moduli_min; num_moduli <= num_moduli_max; ++num_moduli) {
                    ...
                    evaluate_time<isOzaki2>(
                        ...
                        comp_cost,
                        funcname,
                        [&]() { return gemmul8::gemm<T, backend, T, T>(
            ...
            if (run_oz2_f8) {
                for (unsigned num_moduli = num_moduli_min; num_moduli <= num_moduli_max; ++num_moduli) {
                    ...
                    evaluate_time<isOzaki2>(
                        ...
                        comp_cost,
                        funcname,
                        [&]() { return gemmul8::gemmLt<T, backend, T, T>(
                                    handleLt, transa, transb, m, n, k,
                                    &alpha,
                                    A, lda,
                                    B, ldb,
                                    &beta,
            ...
            if (run_oz1_i8) {
                for (auto &num_slice : oz1_slice_list) {
                    ...
                    evaluate_time<isOzaki2>(
                        ...
                        comp_cost,
                        funcname,
                        [&]() { CHECK_CUBLAS(testTraits<T>::gemm(
                                    handle_emu, transa, transb, mi, ni, ki,
                                    &alpha,
            ...
            if constexpr (isDouble && testTraits<T>::is_complex && func == gemmul8::Func::gemm) {
                ...
                const std::string funcname =
                    ...
                    std::to_string(n) + "," +
                    std::to_string(k) + "," +
                    std::string(1, testTraits<T>::prefix_upper) + std::string("GEMM3m");
                ...
                evaluate_time<isOzaki2>(
                    ...
                    comp_cost,
                    funcname,
                    [&]() { CHECK_CUBLAS(testTraits<T>::gemm3m(
                                handle, transa, transb, mi, ni, ki,
                                &alpha,
                                A, static_cast<int64_t>(lda),
                                B, static_cast<int64_t>(ldb),
