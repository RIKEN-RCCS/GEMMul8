...
#include "time_symm.hpp"

namespace bench::time::symm {

inline constexpr gemmul8::Func func = gemmul8::Func::symm;
...
void check_time(
    ...
    std::string fileName   = std::string("oz2_results_") +
                             ...
                             std::string("_time_") +
                             square_tag +
                             ((side == CUBLAS_SIDE_LEFT) ? "left_" : "right_") +
    ...
    PRINT(outFile,
          "phi,m,n,function,TFLOPS,total_time[sec],scaling,low_prec_gemm,mod_hi2mid,undo_scaling,");
    ...
    for (auto &mn : N_list) {
        ...
        for (auto &k : K_list) {
            ...
            // test matrices
            makemat::randmat<T>(k, k, A, phi, seedA, stream, uplo);
            makemat::randmat<T>(m, n, B, phi, seedB, stream);
            ...
            if (run_oz2_i8) {
                for (unsigned num_moduli = num_moduli_min; num_moduli <= num_moduli_max; ++num_moduli) {
                    ...
                    const size_t lwork_gemmul8_i8 = (memory_limit > 0) ? memory_limit : gemmul8::workSize<testTraits<T>::is_complex, gemmul8::Backend::INT8, func>(m, n, k, num_moduli);
                    ...
                    evaluate_time<isOzaki2>(
                        ...
                        comp_cost,
                        funcname,
                        [&]() { return gemmul8::symm<T, backend, T, T>(
            ...
            if (run_oz2_f8) {
                for (unsigned num_moduli = num_moduli_min; num_moduli <= num_moduli_max; ++num_moduli) {
                    ...
                    const size_t lwork_gemmul8_f8 = (memory_limit > 0) ? memory_limit : gemmul8::workSize<testTraits<T>::is_complex, gemmul8::Backend::FP8, func>(m, n, k, num_moduli);
                    ...
                    evaluate_time<isOzaki2>(
                        ...
                        comp_cost,
                        funcname,
                        [&]() { return gemmul8::symmLt<T, backend, T, T>(
                                    handleLt, side, uplo, m, n,
                                    &alpha, A, lda, B, ldb, &beta, C, ldc,
                                    num_moduli, fastmode, work_emu,
            ...
            if (run_oz1_i8) {
                for (auto &num_slice : oz1_slice_list) {
                    ...
                    const size_t lwork_ozaki1 = ozaki1::workSize(m, n, k, 1, testTraits<T>::is_complex, mantissaBitCount);
                    ...
                    evaluate_time<isOzaki2>(
                        ...
                        comp_cost,
                        funcname,
                        [&]() { CHECK_CUBLAS(testTraits<T>::symm(
                                    handle_emu, side, uplo, mi, ni,
...
template void check_time<float>(std::string &, std::string &, size_t, cublasFillMode_t, cublasSideMode_t, const bool, const bool, const bool, const bool);
template void check_time<double>(std::string &, std::string &, size_t, cublasFillMode_t, cublasSideMode_t, const bool, const bool, const bool, const bool);
template void check_time<cuFloatComplex>(std::string &, std::string &, size_t, cublasFillMode_t, cublasSideMode_t, const bool, const bool, const bool, const bool);
template void check_time<cuDoubleComplex>(std::string &, std::string &, size_t, cublasFillMode_t, cublasSideMode_t, const bool, const bool, const bool, const bool);
