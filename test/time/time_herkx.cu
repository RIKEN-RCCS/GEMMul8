...
#include "time_herkx.hpp"

namespace bench::time::herkx {

inline constexpr gemmul8::Func func = gemmul8::Func::herkx;
...
void check_time(
    ...
    static_assert(testTraits<T>::is_complex, "HERKX timing test requires complex T.");

    if (trans == CUBLAS_OP_T) {
        assert(false && "HERKX with trans = T is not supported.");
        return;
    }
            ...
            if (run_oz2_i8) {
                for (unsigned num_moduli = num_moduli_min; num_moduli <= num_moduli_max; ++num_moduli) {
                    ...
                    evaluate_time<isOzaki2>(
                        ...
                        comp_cost,
                        funcname,
                        [&]() { return gemmul8::herkx<T, backend, T, T>(
            ...
            if (run_oz2_f8) {
                for (unsigned num_moduli = num_moduli_min; num_moduli <= num_moduli_max; ++num_moduli) {
                    ...
                    evaluate_time<isOzaki2>(
                        ...
                        comp_cost,
                        funcname,
                        [&]() { return gemmul8::herkxLt<T, backend, T, T>(
            ...
            if (run_oz1_i8) {
                for (auto &num_slice : oz1_slice_list) {
                    ...
                    evaluate_time<isOzaki2>(
                        ...
                        comp_cost,
                        funcname,
                        [&]() { CHECK_CUBLAS(testTraits<T>::herkx(
