...
#include "time_herk.hpp"

namespace bench::time::herk {

inline constexpr gemmul8::Func func = gemmul8::Func::herk;
...
void check_time(
    ...
    static_assert(testTraits<T>::is_complex,
                  "HERK timing test requires complex T.");

    if (trans == CUBLAS_OP_T) {
        assert(false && "HERK with trans = T is not supported.");
        return;
    }
    ...
    for (auto &n : N_list) {
        ...
        for (auto &k : K_list) {
            ...
            size_t lwork_AC                   = lwork_blas + size_A + size_C;
            ...
            const size_t lwork_total = lwork_AC + safety_margin + alignment_margin;
            ...
            if (run_oz2_i8) {
                for (unsigned num_moduli = num_moduli_min; num_moduli <= num_moduli_max; ++num_moduli) {
                    ...
                    evaluate_time<isOzaki2>(
                        ...
                        comp_cost,
                        funcname,
                        [&]() { return gemmul8::herk<T, backend, T>(
            ...
            if (run_oz2_f8) {
                for (unsigned num_moduli = num_moduli_min; num_moduli <= num_moduli_max; ++num_moduli) {
                    ...
                    evaluate_time<isOzaki2>(
                        ...
                        comp_cost,
                        funcname,
                        [&]() { return gemmul8::herkLt<T, backend, T>(
                                    handleLt, uplo, trans, n, k,
                                    &alpha, A, lda, &beta, C, ldc,
                                    num_moduli, fastmode, work_emu,
                                    nullptr, false, false, stream); },
                        stream);
            ...
            if (run_oz1_i8) {
                for (auto &num_slice : oz1_slice_list) {
                    ...
                    evaluate_time<isOzaki2>(
                        ...
                        comp_cost,
                        funcname,
                        [&]() { CHECK_CUBLAS(testTraits<T>::herk(
                                    handle_emu, uplo, trans, ni, ki,
                                    &alpha, A, static_cast<int64_t>(lda),
