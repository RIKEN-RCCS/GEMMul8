...
#include "time_syrk.hpp"

namespace bench::time::syrk {

inline constexpr gemmul8::Func func = gemmul8::Func::syrk;
...
void check_time(
    ...
    if (trans == CUBLAS_OP_C) {
        assert(false && "SYRK with trans = C is not supported.");
        return;
    }
            ...
            if (run_oz2_i8) {
                for (unsigned num_moduli = num_moduli_min; num_moduli <= num_moduli_max; ++num_moduli) {
                    ...
                    evaluate_time<isOzaki2>(
                        ...
                        comp_cost,
                        funcname,
                        [&]() { return gemmul8::syrk<T, backend, T>(
            ...
            if (run_oz2_f8) {
                for (unsigned num_moduli = num_moduli_min; num_moduli <= num_moduli_max; ++num_moduli) {
                    ...
                    evaluate_time<isOzaki2>(
                        ...
                        comp_cost,
                        funcname,
                        [&]() { return gemmul8::syrkLt<T, backend, T>(
            ...
            if (run_oz1_i8) {
                for (auto &num_slice : oz1_slice_list) {
                    ...
                    evaluate_time<isOzaki2>(
                        ...
                        comp_cost,
                        funcname,
                        [&]() { CHECK_CUBLAS(testTraits<T>::syrk(
