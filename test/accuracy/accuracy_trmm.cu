...
#include "accuracy_trmm.hpp"

namespace bench::accuracy::trmm {

inline constexpr gemmul8::Func func = gemmul8::Func::trmm;
...
inline double evaluate_error(
    ...
    auto [err_max, err_med] =
        eval::calc_err(rows, cols, C, ldc, C_exact, ldc_exact, stream);
...
void check_accuracy(
    ...
    for (auto &phi : phi_list) {
        ...
        for (auto &k : K_list) {
            ...
            // Accurate result:
            // side == LEFT  : C_hi := alpha * op(A) * B
            // side == RIGHT : C_hi := alpha * B * op(A)
            eval::DDtrmm<T, T, accu_t>(
