...
void check_accuracy(
    ...
    static_assert(testTraits<T>::is_complex,
                  "HERKX accuracy test requires complex T.");

    if (uplo != CUBLAS_FILL_MODE_UPPER &&
        uplo != CUBLAS_FILL_MODE_LOWER) {
        assert(false && "HERKX requires uplo = UPPER or LOWER.");
        return;
    }
