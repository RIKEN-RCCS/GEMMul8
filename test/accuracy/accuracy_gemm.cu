...
#include "accuracy_gemm.hpp"

namespace bench::accuracy::gemm {
...
inline void check_accuracy(
    ...
    const size_t rowsA_max = (transa == CUBLAS_OP_N) ? m : k_max;
    const size_t colsA_max = (transa == CUBLAS_OP_N) ? k_max : m;

    const size_t rowsB_max = (transb == CUBLAS_OP_N) ? k_max : n;
    const size_t colsB_max = (transb == CUBLAS_OP_N) ? n : k_max;

    const size_t size_A = rowsA_max * colsA_max;
    const size_t size_B = rowsB_max * colsB_max;
