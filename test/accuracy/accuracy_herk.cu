...
#include "accuracy_herk.hpp"

namespace bench::accuracy::herk {
...
void check_accuracy(
    ...
    static_assert(testTraits<T>::is_complex,
                  "HERK accuracy test requires complex T.");

    if (uplo != CUBLAS_FILL_MODE_UPPER &&
        uplo != CUBLAS_FILL_MODE_LOWER) {
        assert(false && "HERK requires uplo = UPPER or LOWER.");
        return;
    }
